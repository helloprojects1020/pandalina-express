        ...
        <div className="mt-6 space-y-6" dir="rtl">
              ...
              {availableCities.length > 0 && (
                <div className="space-y-2">
                  ...
                  {selectedCity && detectedZone && (
                    <div className="bg-green-500/10 border border-green-200 rounded-lg px-3 py-2.5 space-y-1">
                      <p className="text-xs font-semibold text-green-700">📍 {detectedZone.zone_name}</p>
                      <div className="flex items-center gap-4">
                        <span className="text-xs text-green-700">
                          🚚 משלוח: {detectedZone.delivery_fee === 0 ? 'חינם' : `₪${detectedZone.delivery_fee}`}
                        </span>
                        {detectedZone.min_order > 0 && (
                          <span className="text-xs text-green-700">🛒 מינימום: ₪{detectedZone.min_order}</span>
                        )}
                      </div>
                    </div>
                  )}
            ...
            {minOrder > 0 && (
              belowMinimum ? (
                <div className="flex items-center justify-between bg-destructive/10 border border-destructive/20 rounded-lg px-3 py-2">
                  <span className="text-xs font-semibold text-destructive">🛒 מינימום הזמנה: {formatPrice(minOrder)}</span>
                  <span className="text-xs font-bold text-destructive">חסר {formatPrice(minOrder - subtotal)}</span>
                </div>
              ) : (
                <div className="flex items-center justify-between bg-green-500/10 border border-green-200 rounded-lg px-3 py-2">
                  <span className="text-xs font-semibold text-green-700">🛒 מינימום הזמנה: {formatPrice(minOrder)}</span>
                  <span className="text-xs font-bold text-green-700">✓ ניתן להזמין</span>
                </div>
              )
            )}
          ...
          <div className="space-y-3">
            <Button type="button" className="w-full" onClick={handleSend} disabled={submitting || safeItems.length === 0 || restaurantClosed || belowMinimum}>
              {submitting ? "שולח..." : belowMinimum ? `🛒 חסר ${formatPrice(minOrder - subtotal)} למינימום הזמנה` : `📲 שלח הזמנה בוואטסאפ — ${formatPrice(total)}`}
            </Button>
